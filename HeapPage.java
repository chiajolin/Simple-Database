...
public class HeapPage implements Page {
    ...
    public boolean getSlot(int i) {
    	...
    	if(headerIndex >= header.length){
    		return false; //not in the header -> data invalid
    	}
    	int slotHeaderByte = header[headerIndex];
    	int mask = 1 << bitIndex; //big-endian
    	if((mask & slotHeaderByte) > 0){
    		return true;
    	}
	    return false;
	}
    ...
    private void setSlot(int i, boolean value) {
    	...
    	if(getSlot(i) != value){
    		int mask = 1 << bitIndex;
    		slotHeaderByte = slotHeaderByte ^ mask; // ^ : xor , slot bit in header: 1->0 and 0->1
    	}
    }
