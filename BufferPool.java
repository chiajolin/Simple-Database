...
public class BufferPool {
    ...
    private Map<PageId, Page> currentPagesMap;
    private List<String> evictCandidateList;
    ...
    public BufferPool(int numPages) {
    	...
    	this.currentPagesMap = new HashMap<PageId, Page>(); 
    	this.buffer = new HeapPage[numPages];
    	this.buffercount = 0;
    	this.evictCandidateList = new ArrayList<String>();
    }
    ...
    public synchronized Page getPage(TransactionId tid, PageId pid, Permissions perm)
    	...
    	if(currentPagesMap.containsKey(pid)){
    		_numhits ++;
    		pinPage(getBufferIndex((HeapPageId) pid));
    		return  currentPagesMap.get(pid);
    	}
    	//Page is not in the buffer
    		...
    		//add the page into BufferPool
    		//if buffer is full, evict page
    		if(buffercount >= numPages){
    			int evictPageIndex = evictPage();
    			buffer[evictPageIndex] = loadPage;

    			currentPagesMap.remove(buffer[evictPageIndex].id()); 
    		}
    		//if buffer is not full, add the new page to buffer
    		else{
    	   		buffer[buffercount] = loadPage;
    	   		buffercount ++;
    		}
    		//Page pin_count should plus 1 and return the new page
    		pinPage(getBufferIndex((HeapPageId) pid));
	   		currentPagesMap.put(pid, loadPage);
    		return loadPage;
    	}
	}
    ...
    public void pinPage(int index) {
    	//IMPLEMENT THIS
    	buffer[index].pin(); 
    	//delete from candidate list when pin_count != 0
    	if(evictCandidateList.contains(String.valueOf(index))){
    		evictCandidateList.remove(String.valueOf(index));
    	}
    }
    ...
    public void unpinPage(int index, TransactionId tid, boolean dirty) throws DbException, IOException {
    	//IMPLEMENT THIS
    	//if the method unpinPage() is called when the pin_count == 0, it should throws DbException
    	...
    	//add to evictCandidateList if pin_count == 0
    	if(buffer[index].pin_count() == 0){
    		evictCandidateList.add(String.valueOf(index));
    	}
    }
    ...
    */
    ...
    private synchronized int evictPage() throws DbException {
    	//IMPLEMENT THIS
    	//throw exception if buffer pool is full and list of LRU/MRU candidates is empty!
    	if(evictCandidateList.size() == 0){
    		throw new DbException("(getPage()) Buffer pool is full and list of LRU/MRU candidates is empty!");
    	}
    	int evictPageIndex = 0;
    	switch(replace_policy){
    	case LRU_POLICY: //LRU_POLICY
    		evictPageIndex = LRUeviction();
    		break;
    	case MRU_POLICY: //MRU_POLICY
    		evictPageIndex = MRUeviction();
    		break;
    	}
    	return evictPageIndex;
    }
    
    private int LRUeviction() throws DbException{
    	int evictPageIndex = Integer.parseInt(evictCandidateList.get(0));
    	evictCandidateList.remove(0);
    	currentPagesMap.remove(buffer[evictPageIndex].id());
    	return evictPageIndex;
    }
    
    private int MRUeviction() throws DbException{
    	int evictPageIndex = Integer.parseInt(evictCandidateList.get(evictCandidateList.size()-1));
    	evictCandidateList.remove(evictCandidateList.size()-1);
    	currentPagesMap.remove(buffer[evictPageIndex].id());
    	return evictPageIndex;
    }
